...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via KHUNQUANT_KEY_PASSPHRASE
...
//  2. KHUNQUANT_SSH_KEY_PATH env var
//  3. ~/.ssh/khunquant_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "KHUNQUANT_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading KHUNQUANT_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "KHUNQUANT_SSH_KEY_PATH"

// khunquantHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const khunquantHome = "KHUNQUANT_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "khunquant-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (KHUNQUANT_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// KHUNQUANT_SSH_KEY_PATH env var or ~/.ssh/khunquant_ed25519.key.
...
//   - exact match with KHUNQUANT_SSH_KEY_PATH env var
//   - within the KHUNQUANT_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with KHUNQUANT_SSH_KEY_PATH.
	...
	// Within KHUNQUANT_HOME.
	if home := os.Getenv(khunquantHome); home != "" {
		if isWithinDir(clean, home) {
			return true
		}
	}
...
// Final key: HKDF-SHA256(ikm, salt, info="khunquant-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set KHUNQUANT_SSH_KEY_PATH or place key at ~/.ssh/khunquant_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (KHUNQUANT_SSH_KEY_PATH, KHUNQUANT_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. KHUNQUANT_SSH_KEY_PATH env var
//  3. ~/.ssh/khunquant_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the khunquant-specific SSH key path if it exists.
