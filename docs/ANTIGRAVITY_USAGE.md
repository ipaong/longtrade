# Using Antigravity Provider in KhunQuant

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in KhunQuant.
...
```bash
khunquant auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where KhunQuant is waiting.

KhunQuant will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.khunquant/config.json` or override it via the CLI:

```bash
# Override for a single command
khunquant agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Environment Variables**:
    *   `KHUNQUANT_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Authentication persistence**: 
    If you've logged in locally, you can copy your credentials to the server:
    ```bash
    scp ~/.khunquant/auth.json user@your-server:~/.khunquant/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. KhunQuant will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `khunquant auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
