...
const (
	autoStartEntryName = "KhunQuantLauncher"
	launchAgentLabel   = "io.khunquant.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "khunquant-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=KhunQuant Web",
			"Comment=Start KhunQuant Web on login",
